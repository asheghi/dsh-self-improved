...
| M2 | 提取管线：`ctx.llm` 提取 L1 + 严格 JSON 校验/兜底 + 去重 + 防重入节流泵 | ✅ 单元测试通过（11 项，假 LLM 注入）；真实模型调用待用户环境验证 |
| M3 | 召回注入：agent/pre-step 自动注入 + 关键词/向量/混合检索（RRF） | ✅ 单元测试通过 + headless 端到端实测（注入块落入会话记录） |
| M4 | 自进化：L2/L3 归纳（场景+画像版本化）、遗忘衰减、纠正/遗忘工具、技能合成→dsh-skill | ✅ 单元测试通过（15 项，假 LLM）；真实模型效果待用户环境验证 |
| M5 | UI/运维：设置面板（自动渲染）+ 随时开关热应用 + `/memory` 命令 | ✅ 完成（设置命名空间/热应用/命令已实测）；记忆浏览器页面⏳待真实 Web 环境验证（客户端模块约定需在 web profile 下开发） |
